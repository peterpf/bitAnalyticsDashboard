...
const String URL_INSTANCES                  = BASE_URL+"query?query=count(up{job=\"analytics\"}%20==%201)%20by%20(continent)";
const String URL_REQUESTS_PER_MINUTE        = BASE_URL+"query?query=sum(rate(requests_total[1m]))%20by%20(continent)";
const String URL_CPU_USAGE_EU               = BASE_URL+"query?query=avg(node_load1{name=~\".*ingress.*\",%20continent=\"europe\"})";
const String URL_CPU_USAGE_US               = BASE_URL+"query?query=avg(node_load1{name=~\".*ingress.*\",%20continent=\"us\"})";
const int DATA_REFRESH_RATE                 = 10000; // ms

const int NUM_MAX_INSTANCES                 = 10;
const int NUM_MAX_REQUESTS                  = 200;
